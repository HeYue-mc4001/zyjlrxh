...
class AboutAction extends Action {
	...
	**/
	public function _initialize()
	{
		...
		$cate_about = $category->where("pcid = 8")->order("seq,cid")->select();
		$adv = M("adv");
		$now = time();
		$advdata = $adv->where("position = 1 and cids like('%,8,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->find();
		if(!$advdata){
			$advdata['path'] = "Public/images/adv_deafult.jpg";
			$advdata['link'] = "";
		}
		$advdatas = $adv->where("position = 2 and cids like('%,8,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->select();
    ...
    **/
	public function index()
	{
		global $videodata;
		$article = M("article");
		$intro = $article->where("aid = 12")->find();
    ...
    **/
	public function column()
	{
		...
		if($subcategorylist){
			...
			if($articlelist){
				if($is_single==1){
					$this->redirect('about/content?aid='.$articlelist[0]['aid']);
				}else{
					$index = "article".$index;
				}
			}else{
    ...
    **/
	public function content()
	{
		...
		$cname = $category->where("cid = $cid")->getField("cname");
		$train = M('train');
		$now = time();
		$train_n = $train->where("duedate > $now")->order('seq,aid')->select();
    ...
    **/
	public function honor()
	{
		...
		$honor = M("slide");
		$honordata = $honor->where("id = $id")->find();
				
		$this->assign("honordata",$honordata);
		$this->display("about:honor");
    }
