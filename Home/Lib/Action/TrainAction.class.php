...
class TrainAction extends Action {
	...
	**/
	public function _initialize()
	{
		...
		$adv = M("adv");
		$now = time();
		$advdata = $adv->where("position = 1 and cids like('%,3,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->find();
		if(!$advdata){
			$advdata['path'] = "Public/images/adv_deafult.jpg";
			$advdata['link'] = "";
		}
		$advdatas = $adv->where("position = 2 and cids like('%,3,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->select();
		...
		$video = M("video");
		$videodata = $video->where("cid = 3")->order("seq,id desc")->find();
		if(!$videodata){
    ...
    **/
	...
	{
		$slide = M("slide");
		$slidelist = $slide->where("cid = 3 and status = 1")->order('seq,id desc')->select();
		
		$train = D('train');
		$now = time();
		$train_n = $train->where("duedate > $now")->order('seq,aid')->top9();
		$train_hot = $train->where("duedate < $now")->order('seq,aid desc')->top9();
		$train_n = $this->isNew($train_n);
		$train_hot = $this->isNew($train_hot);
		
		$tr = $this->getAllCategory(3);
		$trainimg = $train->field("aid,title,pic")->where("cid in ($tr) and pic <> ''")->order('aid desc')->select();
						
		$this->assign("trainimg",$trainimg);
		$this->assign("slidelist",$slidelist);
		$this->assign("train_n",$train_n);
		$this->assign("train_hot",$train_hot);
		$this->display("train:index");
    }
    ...
    **/
    ...
    {   	
    	...
    	$validate = array(
			array('linkman','require','姓名不能为空！',1),
			array('phone','require','电话不能为空！',1),
        	array('phone','number','电话必须为数字！',1),
		);
        $enroll->setProperty('_auto', $auto);
        $enroll->setProperty('_validate', $validate);
		if($enroll->Create()){
			if($_POST['message']=="如有需要，在此留下你想说的话！"){
				$enroll->message = "";
			}			
			$enroll->add();
			$this->success('新增成功');
    ...
    **/
	public function column()
	{
		...
		$train = M("Train");
		$articlelist = $train->where("cid = $pcid")->select();
		...
		if($subcategorylist){
			...
			if($articlelist){
				if($is_single==1){
					$this->redirect('train/content?aid='.$articlelist[0]['aid']);
				}else{
					$index = "article".$index;
				}
			}else{
		...
		$p_cate_title = $category->where("cid = $p_pcid")->getField("cname");$cids = $this->getAllCategory($p_pcid);
		
		$con_list = $train->field("aid,cid,title,dateline")->where("(cid in ($cids)) and (cid != $pcid)")->order('aid desc')->select();
    ...
    **/
	public function content()
	{
		...
		$train = M("train");
		$articlecontent = $train->where("aid = $aid")->find();
		if(!$articlecontent){
			echo "该项培训不存在或已经被删除！";
		}
		global $category,$videodata;
		...
		$art_other = $train->field("aid,title")->where("cid = $cid and aid!= $aid")->order("seq,aid desc")->select();
		...
		$con_list = $train->field("aid,cid,title,dateline")->where("(cid in ($cids)) and (cid != $pcid)")->order('seq,aid desc')->select();
		
		$train = M('train');
		$now = time();
		$train_n = $train->where("duedate > $now")->order('seq,aid')->select();
		
		if($articlecontent['duedate']>$now){
			$isshow = 1;
		}else{
			$isshow = 0;
		}
