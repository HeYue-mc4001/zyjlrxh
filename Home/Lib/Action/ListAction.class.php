...
class ListAction extends Action {
	...
	**/
	public function _initialize()
	{
		...
		$adv = M("adv");
		$now = time();
		$advdata = $adv->where("position = 1 and cids like('%,4,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->find();
		if(!$advdata){
			$advdata['path'] = "Public/images/adv_deafult.jpg";
			$advdata['link'] = "";
		}
		$advdatas = $adv->where("position = 2 and cids like('%,4,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->select();
		...
		$video = M("video");
		$videodata = $video->where("cid = 4")->order("seq,id desc")->find();
		if(!$videodata){
    ...
    **/
	...
	{	
		$slide = M("slide");
		$slidelist = $slide->where("cid = 4 and status = 1")->order('seq,id desc')->select();
			
		$entre = M("entre");
		$entrelist = $entre->where("pic <> ''")->order("seq,id desc")->select();
		
		$article = D('article');
		$article_n = $article->where("cid = 29")->order('seq,aid desc')->top9();
		$article_n = $this->isNew($article_n);
		
		$article_v = $article->where("cid = 31")->order('seq,aid desc')->top4();
		
		$this->assign("slidelist",$slidelist);
		$this->assign("article_n",$article_n);
		$this->assign("article_v",$article_v);
    ...
    **/
	public function column()
	{
		...
		if($subcategorylist){
			...
			if($articlelist){
				if($is_single==1){
					$this->redirect('list/content?aid='.$articlelist[0]['aid']);
				}else{
					$index = "article".$index;
				}
			}else{
    ...
    **/
	public function entre()
	{
		...
		$entre = M("entre");
		$entredata = $entre->where("id = $id")->find();
		
		$article = M("article");
		$aid = $entredata['aid'];
		$entredata['a_name'] = $article->where("aid = $aid")->getField("title");
		if(!$entredata['pic']){
			$entredata['pic'] = "Public/images/nopic.jpg";
		}
		
		$this->assign("entredata",$entredata);
		$this->display("list:entre");
    }
    ...
    **/
	public function content()
	{
		...
		$entre = M("entre");
		$entrelist = $entre->field("id,name,company,position,area")->where("aid = $aid")->order("seq,id")->select();
