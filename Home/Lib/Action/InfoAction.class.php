...
class InfoAction extends Action {
	...
	**/
	public function _initialize()
	{
		...
		global $noticelist,$category,$videodata;
		$category = M('category');
		...
		$adv = M("adv");
		$now = time();
		$advdata = $adv->where("position = 1 and cids like('%,1,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->find();
		if(!$advdata){
			$advdata['path'] = "Public/images/adv_deafult.jpg";
			$advdata['link'] = "";
		}
		$advdatas = $adv->where("position = 2 and cids like('%,1,%') and dates < $now and (datee > $now or datee = 0)")->order("seq,id desc")->select();
		...
		$video = M("video");
		$videodata = $video->where("cid = 1")->order("seq,id desc")->find();
		if(!$videodata){
    ...
    **/
	...
	{	
		global $videodata;
		$slide = M("slide");
		$slidelist = $slide->where("cid = 1 and status = 1")->order('seq,id desc')->select();
				
		$article = D('article');
		$article_n = $article->where("cid = 7")->order('seq,aid desc')->top9();
		$article_m = $article->where("cid = 10")->order('seq,aid desc')->top9();
		$article_t = $article->where("cid = 11")->order('seq,aid desc')->top9();
		$article_g = $article->where("cid = 13")->order('seq,aid desc')->top9();
		$article_ma = $article->where("cid = 14")->order('seq,aid desc')->top9();
		$article_s = $article->where("cid = 31")->order('seq,aid desc')->top9();
		$article_n = $this->isNew($article_n);
		$article_m = $this->isNew($article_m);
		$article_t = $this->isNew($article_t);
		$article_g = $this->isNew($article_g);
		$article_ma = $this->isNew($article_ma);
		$article_s = $this->isNew($article_s);
		
		$info = $this->getAllCategory(1);
		
		$artimg = $article->field("aid,title,pic")->where("cid in ($info) and pic <> ''")->order('aid desc')->select();
						
		$this->assign("artimg",$artimg);
		$this->assign("videodata",$videodata);
		$this->assign("slidelist",$slidelist);
		$this->assign("article_n",$article_n);
		$this->assign("article_m",$article_m);
		$this->assign("article_t",$article_t);
		$this->assign("article_g",$article_g);
		$this->assign("article_ma",$article_ma);
    ...
    **/
	public function column()
	{
		...
		$category = M("Category");
		$categorydata = $category->field("cid,pcid,cname,module")->where("cid = $pcid")->find();
		...
		$article = M("Article");
		$articlelist = $article->where("cid = $pcid")->order("seq,aid desc")->select();
		...
		if($subcategorylist){
			...
			if($articlelist){
				if($is_single==1){
					$this->redirect(MODULE_NAME.'/content?aid='.$articlelist[0]['aid']);
				}else{
					$index = "article".$index;
				}
			}else{
