    ...
    public function index()
    {
		...
		if($_POST['start']){
			...
			if($_POST['end']){
				...
				if($t1 < $t2){
					if($where){
    	    			$where.=" AND (regdate >= $t1 AND regdate <= $t2)";
    				}else{
    					$where.="(regdate >= $t1 AND regdate <= $t2)";	
    				}
				}
			}else{
				if($where){
    	    		$where.=" AND (regdate >= $t1)";
    			}else{
    				$where.="(regdate >= $t1)";	
    			}
			}
    	...
    	$memberlist=$member->where($where)->limit($p->firstRow.','.$p->listRows)->select();
		foreach($memberlist as $key=>$value){
			if($value['is_checked']==1){
				$memberlist[$key]['is_checked'] = "审核通过";
			}else{
				$memberlist[$key]['is_checked'] = "尚未审核【<a href=\"__URL__/audit/ids/".$value['id']."\">快速审核</a>】";
			}
		}
    	$page = $p->show ();
