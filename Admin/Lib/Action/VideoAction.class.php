...
class VideoAction extends Action
{
	...
	**/
	public function _initialize()
	{
		...
		global $cate,$video,$cids,$privilege;
    ...
    **/
    ...
    {
		global $video,$cate,$cids,$privilege;
    	...
    	$videolist=$video->where($where)->order("seq asc,id desc")->limit($p->firstRow.','.$p->listRows)->select();
    	$page = $p->show ();
    ...
    **/
    ...
    {
    	global $video,$cateoption,$privilege;
    	$privilege->checkPrivilege($_SESSION['admin']['id'], MODULE_NAME."_".ACTION_NAME);
		
		$id=$_GET['id'];
    	$videoList=$video->where("id=$id")->find();
    ...
    */
    public function addaction()
    {
		...
		$video->setProperty('_validate',$validate);
		if($video->Create()){
			if($info[0]["savename"]){
				$video->path=$info[0]["savepath"].$info[0]["savename"];
			}
			$video->add();
